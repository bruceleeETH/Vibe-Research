...
- 当前实查规模（2026-09-11）：全 A 快照 5,913 只，主板 3,487 只，按当前名称排除 ST/退市整理后 3,116 只；其中总市值不低于 30 亿元 2,688 只，低于门槛 364 只，市值缺失 64 只。
...
首期 3 个月 MVP 暂以 2026-09-11 当前快照筛出的 2,688 只作为固定采集清单，并明确标记 `universe_mode=current_snapshot`、`market_cap_asof=2026-09-11`。该模式只用于验证采集、存储、查询和页面，不得作为无偏历史回测结论。正式研究必须切换到 `universe_mode=point_in_time`。
...
- 当前主板非 ST 且总市值不低于 30 亿元的股票池数量与东财快照差异可解释；基准日为 2,688 只，另有 64 只市值缺失被单列。
...
| 范围 | 约行数 | DuckDB 预估 | 用途 |
|---|---:|---:|---|
| 当前 2,688 只 × 3 个月 + 30 日预热 | 约 24 万 | 20–60 MB | 首期验收 |
